        ...
        with conn.cursor() as cur:
            if condominio_slug:
                ...
                cur.execute("""SELECT u.id, u.condominio_id, u.nome, u.usuario, u.senha, u.nivel, u.ativo
                               FROM usuarios u
                               JOIN condominios c ON c.id=u.condominio_id
                               WHERE u.usuario=%s AND c.ativo=TRUE
                               ORDER BY u.id LIMIT 2""", ((usuario or "").strip(),))
                rows=cur.fetchall()
...
def listar_moradores(apenas_ativos=True):
    ...
    try:
        with conn.cursor() as cur:
            filtro = "AND m.ativo = TRUE" if apenas_ativos else ""            query = (  # nosec B608 - filtro is a local boolean-controlled constant
                f"""
                SELECT
